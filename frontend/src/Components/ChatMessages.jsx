...
const ChatApp = ({ userId, chatroomid, currentRoom, setCreator, creator, chatid, chatRooms = [] }) => {
  ...
  const noChatSelected = !currentRoom;

  const handleSend = () => {
    if (noChatSelected) return;
    if (inputValue.trim()) {
      const userMessage = {
        text: inputValue,
        room: chatroomid,
  ...
  return (
    <div className="h-auto w-full bg-green-200 right-5 rounded-lg">
      <Card className="flex-grow w-full z-100 mt-10">
            ...
            {messages.map((message, index) => {
              ...
              return (
                <div
                  ...
                  className={`mb-2 w-fit  p-3 rounded-lg relative flex items-center gap-2 ${
                    isCurrentUser
        ...
        <CardContent className="flex gap-2 p-3">
          <Input
            ...
            placeholder={noChatSelected ? "Select a chat to start messaging" : "Type a message..."}
            disabled={noChatSelected}
          />
          <Button
